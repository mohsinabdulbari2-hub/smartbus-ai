...
const CROWD_MAP: Record<string, { variant: any; emoji: string }> = {
  Low:    { variant: "success", emoji: "🟢" },
  Medium: { variant: "warning", emoji: "🟡" },
  High:   { variant: "danger",  emoji: "🔴" },
};

export function CrowdBadge({ level, size }: { level: "Low" | "Medium" | "High"; size?: "sm" | "md" }) {
  const c = CROWD_MAP[level] || CROWD_MAP.Low;
  return <Badge variant={c.variant} emoji={c.emoji} label={level} size={size ?? "sm"} />;
}
